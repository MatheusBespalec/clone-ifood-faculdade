<div>
    ...
    <div
        ...
        class="fixed w-full max-w-lg h-5 bg-white top-20 h-full border border-gray-300 right-0">
        <x-icon.close @click="cartOpen = !cartOpen" class="text-red-600 w-5 absolute top-2 left-2 cursor-pointer"/>
        <div x-show="$wire.totalItems !== 0" class="flex flex-col justify-between h-full">
        ...
        <div x-show="$wire.totalItems === 0"
            class="text-center h-full flex items-center justify-center flex-col"
        >
            <h3 class="font-medium">Sua sacola está vazia</h3>
            <p class="text-sm text-gray-500">Adicione itens</p>
        </div>
    </div>
</div>
